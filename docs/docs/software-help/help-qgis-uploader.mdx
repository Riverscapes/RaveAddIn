...
Use the `Add` and `Remove` buttons to add tags (8) to the project in the Data Exchange. Projects that originated from the Data Exchange pre-populate the list with the existing tags.
