---
title: Web Viewer Vector Symbology
description: Instructions for defining Web Viewer vector symbology
---

The Riverscapes Viewer for the web uses symbology defined as [JSON](https://en.wikipedia.org/wiki/JSON), a text format common across the web. This is unlike Viewer for QGIS that uses QML files and ArcGIS that uses layer files.

## Overall structure of the JSON file

The file defining the symbology for a vector layer in the Web Viewer should have the following three-part structure:
...
* `"$schema"` This points to the schema definition file. It's optional but if you use it then vscode will be able to tell you if there are problems with the file.
...
## Recommended Process to build a Web Viewer symbology file
...
As mentioned, the Riverscapes Viewer for the web uses symbology defined as [JSON](https://en.wikipedia.org/wiki/JSON), a text format common across the web. This is unlike Viewer for QGIS that uses QML files and ArcGIS that uses layer files.

The JSON for the layerStyles section is somewhat complicated to write by hand, so the following workflow uses an online tool called MapBox to configure it within a user interface and from there export the JSON needed for Web Viewer.
...
If you don't want to use MapBox, you can also use the Symbology Tool that is included in the Riverscapes Viewer for the Web, available by right-clicking on any layer in a project, as shown:

![context menu for layer in Riverscapes Map Viewer](/images/webviewer/webviewer-ss-symbology-tool.png)

## Before You Start

You should have the following available before you begin:

1. A data layer for which you want to define the Web viewer symbology.
2. An idea of how you want the layer in question to be symbolized. This will most likely be informed from symbology that has already been defined for QGIS Viewer or ArcGIS viewer.
3. [QGIS](https://qgis.org/en/site/) Desktop GIS software.
4. A free [MapBox](https://studio.mapbox.com) account.
5. A powerful text editor, such as [Visual Studio Code](https://code.visualstudio.com/).

## Prepare Your Data

1. Start QGIS and load the layer you want to symbolize into the map. If possible, use the [Viewer](/software-help/help-qgis) for QGIS to help you do this. In other words, download a project that contains the layer in question and add it to the current map document by double clicking it in the viewer project tree.
...
1. Click the word "Blank" in the top bar to make the name editable and provide a good name (this name is used by Viewer, but is useful because this style is going to persist in your MapBox account and you might need to find it in the future).
...
1. Now it's time to symbolize your layer. Some guidelines are below:
    ...
    - Search for online tutorials or videos. MapBox is well supported.
...
The `.json` files for vector symbols should live in the same folder as the `.txt` files defining [symbology for rasters](symbology-web-rasters).
