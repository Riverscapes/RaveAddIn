...
The JSON in question is somewhat complicated to write it by hand, so the following workflow  uses an online tool called MapBox to configure it within a user interface and from here export the JSON needed for Web Viewer.
...
Let's summarize this overall structure. There are 3 parts to this file:
