...
description: How to implement map symbology in ArcViewer
position: 5
---

:::warning

This is a deprecated page, the RAM Lab no longer updates symbology for the ArcMap Viewer. See [ArcPro Viewer Symbology](symbology-arcpro) for curation documentation for ArcGIS Pro.

:::

ArcViewer stores symbology in proprietary ESRI layer files. Each type of RAVE dataset has its own layer file.

ESRI Layer files contain two bits of information. First they possess a path to a specific GIS dataset on your computer and second they store the definition of how the dataset should be symbolized. RAVE only uses the latter and ignores the path to the dataset itself. In this way RAVE can apply the symbology stored in a single layer file to many layers of the same type. See the [video demonstration below](#video-demonstration) to see how all of this works.
