...
| Singleband pseudocolor (clamped) | `INTERPOLATION:DISCRETE` | Smooth gradient between stops; values below the lowest or above the highest stop are clamped to the edge color | Continuous data where the color ramp range should stay fixed (slope, aspect, bathymetry) |
...
:::note `INTERPOLATION:DISCRETE` is not for categorical data
